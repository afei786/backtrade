...
class StockBacktest:
    def __init__(self, config_file="settings.yaml"):
        ...
        self.position_log = {}  # 持仓历史记录 'stock_code': {'is_position': True, 'position': 100, 'cost_price': 10.0,'price': 0.0, 'profit': 0.0}
        ...
        # self.end_time = self.data['trade_date'].max()
    ...
    def get_day_data(self, date=None):
        # if not self.stock_list:
        #     print(f"没有设置stock_list, 无法获取日数据")
        #     return None
        # 获取所有符合条件的股票代码
        where_clause = ["open <= %s", "market_cap < %s",]
        params = [self.settings['ma_line'], self.settings['max_market_cap']]

        if self.settings['region'] is not None:
            placeholders = ', '.join(['%s'] * len(self.settings['region']))
            where_clause.append(f'region in ({placeholders})')  # 筛选指定地域
            params.extend(self.settings['region'])

        if self.settings['not_market_type'] is not None:
            placeholders = ', '.join(['%s'] * len(self.settings['not_market_type']))
            where_clause.append(f'market_type not in ({placeholders})')  # 排除板块
            params.extend(self.settings['not_market_type'])

        # 构建完整的WHERE子句
        where_str = " AND ".join(where_clause)

        stock_list = self.sql.select(
            'stock_info',
            columns=['stock_code'],
            where=where_str,
            params=params
        ...
        if not stock_codes:
            print("没有符合条件的stock")
            return []
        print(f"筛选出 {len(stock_codes)} 只stock")
        ...
        placeholders = ', '.join(['%s'] * len(self.stock_list))
        if date is None:
            where_clause = f'trade_date = "{self.current_date}" AND stock_code IN ({placeholders})'
        else:
            where_clause = f'trade_date = "{date}" AND stock_code IN ({placeholders})'
    ...
    def buy(self, stock: str, price: float, amount: int, additional: bool = False):
        ...
        if stock not in self.stocks_position:
            self.stocks_position[stock] = {'available': 0, 'unavailable': 0, 'cost_price': 0, 'close_price': 0, 'buy_date': self.current_date}
        self.stocks_position[stock]['unavailable'] = amount
        
        # 计算成本价
        if self.stocks_position[stock]['available'] == 0:
    ...
    def sell(self, stock: str, price: float, amount: int):
        ...
        if amount == -1:
            ...
            self.position_log[stock] = {
                ...
                'cost_price': f"{self.stocks_position[stock]['cost_price']:.2f}",
                'price': price,
                'profit': f"{profit:.2f}",
            ...
            self.zy_dict[stock] = {'price': self.close_price, 'time': self.current_date}  # 记录止盈股票
        else:
    ...
    def next(self):
        ...
        current_data = self.get_day_data()
        ...
        if not current_data.empty:
            ...
            if len(self.stocks_position) > 0:
                ...
                # 
                zy_dict_keys = list(self.zy_dict.keys())
                for stock in zy_dict_keys:
                    if self.current_date - self.zy_dict[stock]['time'] > timedelta(days=30):
                        del self.zy_dict[stock]  # 删除止盈股票记录
            ...
            for stock in list(self.stocks_position.keys()):  # 使用列表副本遍历，避免在遍历过程中修改字典结构
                ...
                self.log_message(f"{stock} 持仓: {available_amount}, 成本价：{cost_price:.2f}, 收盘价：{close_price:.2f}, 当日盈亏：{day_profit:.2f}, 累计盈亏：{profit:.2f}, 累计收益率: {profit_rate:.2f}%, 买入日期：{datetime.strftime(self.stocks_position[stock]['buy_date'], '%Y-%m-%d')}")
                self.position_log[stock] = {
                    'is_position': True,
            ...
            self.log_message(f"止盈股票数量: {len(self.zy_dict)}")
            ...
            self.end_time1 = self.current_date
    ...
    def _apply_strategy(self, current_data):
        ...
        for stock in self.stock_pool:
            ...
            self.zy_ma = float(stock_data[self.settings['zy_ma_line']].values[0])
            # if self.cash > 50000 and len(self.stocks_position) > self.max_stock_num:
            #     self.strategy(stock, 200)
            # else:
            self.strategy(stock, 100)


    def check_position(self, stock):
        """检查持仓情况"""
        if stock in self.stocks_position:
            if (self.current_date - self.stocks_position[stock]['buy_date']).days > 30 and self.open_price/self.stocks_position[stock]['cost_price'] > 1.1:
    ...
    def strategy(self,stock,amount=100):
        """
        盘中买入策略
        """
        if stock not in self.stocks_position and stock not in self.zy_dict:  # 如果股票不在持仓中且不在止盈列表中
            if self.settings['min_price'] <= self.open_price <= self.settings['max_price'] and self.open_price <= self.ma:  
                self.buy(stock, self.open_price, amount)  # 建仓
            elif self.settings['min_price'] <= self.low_price <= self.settings['max_price'] and self.low_price <= self.ma:
                self.buy(stock, self.low_price+0.1, amount)  # 建仓

        elif stock in self.zy_dict:  # 在止盈列表中
            if self.settings['min_price'] <= self.open_price <= self.settings['max_price'] and self.open_price <= self.zy_ma :
                self.buy(stock, self.open_price, amount)  # 建仓
                del self.zy_dict[stock]  # 删除止盈股票记录

            elif self.settings['min_price'] <= self.low_price <= self.settings['max_price'] and self.low_price <= self.zy_ma:
                self.buy(stock, self.low_price+0.1, amount)  # 建仓
                del self.zy_dict[stock]  # 删除止盈股票记录
        ...
        if self.show_progress:
            ...
            with tqdm(total=total_days, desc="回测进度", unit="天") as pbar:
                while self.current_date <= self.end_time:
                    ...
                    pbar.set_postfix(收益率=f"{self.profit_rate:.2f}%",
                                     资金=f"{self.cash:.2f}", 
    ...
    def close_log(self):
        ...
        sorted_position = sorted(
            self.position_log.items(),
            key=lambda x: float(x[1]['profit']),
            reverse=True
        )
        self.position_log = {stock: info for stock, info in sorted_position}
        for stock in self.position_log:
            
            self.log.write(f"{stock} 是否持仓: {self.position_log[stock]['is_position']}, 持仓数：{self.position_log[stock]['position']}, 成本价：{self.position_log[stock]['cost_price']}, ")
        ...
        self.log.write("===========================================\n")
        end_data = self.get_day_data(self.end_time1)
        
        for stock in self.zy_dict:
            stock_data = end_data[end_data['stock_code'] == stock]
            if stock_data.empty:
                continue
            self.open_price = float(stock_data['open'].values[0])
            self.close_price = float(stock_data['close'].values[0])

            self.log.write(f"{stock} 止盈价: {self.zy_dict[stock]['price']:.2f}, 止盈时间: {datetime.strftime(self.zy_dict[stock]['time'], '%Y-%m-%d')}, 当前收盘价：{self.close_price}")
            self.log.write(f"，止盈后涨跌: {(self.close_price - self.zy_dict[stock]['price'] ):.2f}\n")
        self.log.write("===========================================\n")
        self.log.write("回测结束\n")
        self.log.close()
